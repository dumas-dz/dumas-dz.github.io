...
LangChain本身集成了大量开箱即用的文档加载器（Document Loader），覆盖主流文档格式，无需手动编写解析逻辑，只需调用对应加载器，即可将文档内容转换为LangChain统一的Document对象（包含文本内容和元数据），便于后续统一处理。
...
所有加载器加载文档后，都会返回一个List[Document]（Document对象列表），每个Document对象包含两个核心属性，是后续处理的基础：
...
代码来源：LangChain PyPDFLoader官方示例（python.langchain.com/docs/module…）；
...
代码来源：LangChain Docx2txtLoader官方示例（[python.langchain.com/docs/module…python.langchain.com/docs/module… word#docx2txt)）；

依赖安装：pip install docx2txt；

替代方案：若需要提取Word中的表格、图片，可使用UnstructuredWordLoader（依赖pip install unstructured）。
...
代码来源：LangChain UnstructuredLoader官方示例（python.langchain.com/docs/module…）；

依赖安装：pip install unstructured（若需要处理图片，需额外安装pip install pillow）；

核心优势：兼容复杂排版，支持提取图片、表格、段落等元素，适合对文档结构要求高的场景；劣势：速度比PyPDF、Docx2txt慢，资源消耗略高。

### 7.2.4 加载网页HTML（URLLoader）

除了本地文档，开发中常需要加载网页内容（如爬取网页文档进行分析），LangChain的URLLoader可直接通过URL加载网页，自动提取文本内容，去除HTML标签冗余。
...
自定义Document Loader的核心很简单：继承LangChain的BaseLoader类，重写两个方法即可实现自定义加载逻辑。

### 7.3.1 自定义Loader的核心步骤

所有自定义加载器都需继承langchain.document_loaders.base.BaseLoader，并重写以下两个方法：

- load(self) -> List[Document]：核心方法，实现文档加载逻辑，返回Document对象列表；

load(self) -> List[Document]：核心方法，实现文档加载逻辑，返回Document对象列表；

- load_and_split(self, text_splitter: Optional[TextSplitter] = None) -> List[Document]：可选方法，加载文档后直接进行文本分割，返回分割后的Document列表（可复用父类方法，无需重写）。
...
代码来源：基于LangChain BaseLoader自定义开发（参考官方自定义加载器文档：python.langchain.com/docs/module…）；
...
LangChain提供了多种文本分割器，其中最常用的是CharacterTextSplitter和RecursiveCharacterTextSplitter，两者适用场景不同，本节重点对比两者的区别和用法。
...
代码来源：LangChain CharacterTextSplitter官方示例（python.langchain.com/docs/module…）；
...
代码来源：LangChain RecursiveCharacterTextSplitter官方示例（python.langchain.com/docs/module…）；
...
代码来源：LangChain NLTKTextSplitter官方示例（python.langchain.com/docs/module…）；

替代方案：若需要更精准的中文句子分割，可使用SpacyTextSplitter（依赖pip install spacy，并下载中文模型python -m spacy download zh_core_web_sm）。
...
代码文件（如.py、.java）的核心是“代码块、函数、类”，分割时需保留代码结构（如函数完整、语法正确），LangChain提供LanguageParser（语言解析器），可按代码语法分割。
...
代码来源：LangChain LanguageParser官方示例（python.langchain.com/docs/module…）；
...
表格数据（如Excel、CSV）的核心是“行、列”，加载时需保留表格结构，分割时需避免破坏行数据的完整性，常用PandasExcelLoader加载，再按行或按表格分割。
...
代码来源：LangChain PandasExcelLoader官方示例（python.langchain.com/docs/module…）；
