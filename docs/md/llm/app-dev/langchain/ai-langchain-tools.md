...
在LangChain中，「Tool」是一个抽象概念，本质是可被LLM调用的“功能模块”——它可以是简单的Python函数、外部API接口、第三方工具（如搜索引擎、计算器），也可以是复杂的业务逻辑（如订单查询、数据统计）。
...
- 获取实时信息：如查询当前天气、最新新闻、股票价格；

获取实时信息：如查询当前天气、最新新闻、股票价格；

- 执行计算操作：如计算行程时间、金额换算、数据统计；

执行计算操作：如计算行程时间、金额换算、数据统计；

- 与外部系统交互：如调用API、操作数据库、发送消息；

与外部系统交互：如调用API、操作数据库、发送消息；

- 执行自定义逻辑：如业务流程查询、用户偏好匹配、权限校验。
...
引用来源：LangChain Tools 官方文档、LangChain 工具调用实战详解。
...
引用来源：LangChain 内置工具集成文档、LangChain 内置工具实战。
...
引用来源：LangChain Pydantic工具输入输出文档、LangChain 工具参数校验实战。

## 12.5 工具描述（description）的重要性

在LangChain工具调用中，「工具描述（description）」是最容易被忽略，但却是最关键的部分——LLM完全依赖工具描述，判断“是否需要调用该工具”“如何调用该工具”“传递什么参数”。
...
- 工具功能：清晰说明工具能做什么（如“查询指定城市的实时天气”“计算两个城市之间的行程时间”）；

工具功能：清晰说明工具能做什么（如“查询指定城市的实时天气”“计算两个城市之间的行程时间”）；

- 输入参数：说明需要输入的参数（名称、类型、含义），若有多个参数，需分别说明（如“输入参数为出发城市（字符串）和到达城市（字符串）”）；

输入参数：说明需要输入的参数（名称、类型、含义），若有多个参数，需分别说明（如“输入参数为出发城市（字符串）和到达城市（字符串）”）；

- 适用场景：说明什么时候需要调用该工具（如“用户询问具体城市天气时调用”“用户需要计算行程时间时调用”）；

适用场景：说明什么时候需要调用该工具（如“用户询问具体城市天气时调用”“用户需要计算行程时间时调用”）；

- 返回结果：简要说明工具返回的内容格式（如“返回天气状况和温度范围”“返回行程时间（分钟）和路线建议”）。
...
- 参数校验：用Pydantic过滤无效、恶意参数，防止注入；

参数校验：用Pydantic过滤无效、恶意参数，防止注入；

- 权限校验：给工具添加角色权限，防止越权调用；

权限校验：给工具添加角色权限，防止越权调用；

- 频率限制：控制工具调用频率，防止恶意调用。

频率限制：控制工具调用频率，防止恶意调用。

引用来源：LangChain 安全最佳实践文档。
